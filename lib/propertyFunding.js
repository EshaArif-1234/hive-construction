...
/** Active investments on a property (excludes withdrawn/completed exits). */
export async function getPropertyActiveInvestors(propertyId) {
  const oid = toObjectId(propertyId);
  if (!oid) return [];

  const rows = await Investment.find({ propertyId: oid, status: "active" })
    .populate("investorId", "fullName")
    .sort({ investmentDate: -1 })
    .select("amount sharePercentage investmentDate investorId")
    .lean();

  return rows.map((row) => ({
    id: String(row._id),
    investorId:
      row.investorId && typeof row.investorId === "object"
        ? String(row.investorId._id || "")
        : String(row.investorId || ""),
    investorName:
      row.investorId && typeof row.investorId === "object" && row.investorId.fullName
        ? String(row.investorId.fullName)
        : "Investor",
